...
#define MIN_TIME 70
#define MIN_GAP 3000
#define MAX_SEQUENCE MAX_PULSES / 2
#define MAX_TIMES 16
...
bool config_load_head();
bool config_load_frame();
void config_save_head();
...
void setup() {
    ...
    if (config_load_head()) {
    	...
    	CLEAR(config_flags, FLAGS_FRAME);
    	config_save_head();
    }
...
bool config_load_head() {
	config_head_t head;
	eeprom_read_block((void*) &head, (const void*) CONFIG_OFFSET, sizeof(config_head_t));
	
	if (head.magic != MAGIC) return false;
	if (head.version != VERSION) return false;
	
	config_flags = head.flags;
	
	return true;
}

bool config_load_frame() {
	if (!GET(config_flags, FLAGS_FRAME)) return false;
	
	eeprom_read_block((void*) &frame, (const void*) (CONFIG_OFFSET + sizeof(config_head_t)), sizeof(frame_t));
...
void config_save_head() {
	...
	head.flags = config_flags;
	
	eeprom_update_block((void*) &head, (void*) CONFIG_OFFSET, sizeof(config_head_t));
}

void config_save_frame() {
	eeprom_update_block((void*) &frame, (void*) (CONFIG_OFFSET + sizeof(config_head_t)), sizeof(frame_t));
}
...
void send(const frame_t* frame, uint8_t repeat) {
	if (!GET(config_flags, FLAGS_FRAME)) {
